...
async def alert_webhook(payload: AlertWebhook, db: Session = Depends(get_db)):
    ...
    add_event(
        ...
        incident.id, 
        "alert_received", 
        f"Alert received from {payload.source}", 
        payload.description, 
        actor_type="system", 
        data=payload.model_dump()
    )
    
    # --- Yahan Triage Agent add kiya hai ---
    triage_result = await triage_alert(payload.model_dump())
    add_event(
        db,
        incident.id,
        "triage_completed",
        "Triage Agent classified alert",
        f"Classified as {triage_result.get('alert_category')} with {triage_result.get('severity')} severity",
        actor_type="agent",
        data=triage_result,
    )
    # ---------------------------------------
